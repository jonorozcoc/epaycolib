        ...
        try {
            ...
            if ($method == 'GET') {
                if ($switch) {
                    ...
                    $response = \Requests::get(self::BASE_URL_SECURE . $url, $headers, $options);
                } else {
                    $response = \Requests::get(self::BASE_URL . $url, $headers, $options);
                }
            } elseif ($method == 'POST') {
                if ($switch) {
                    $data = $util->mergeSet($data, $test, $lang, $private_key, $api_key, $cash);

                    $response = \Requests::post(self::BASE_URL_SECURE . $url, $headers, json_encode($data), $options);
                } else {
                    if ($card) {
                ...
                if ($safetyp) {
                    ...
                    $data = $util->mergeSet($data, $test, $lang, $private_key, $api_key, $cash);
                    $response = \Requests::post(self::BASE_URL_SECURE . $url, $headers2, $data, $options);
                }
            } elseif ($method == 'DELETE') {
                $response = \Requests::delete(self::BASE_URL . $url, $headers, $options);
            }
        } catch (\Exception $e) {
